![alt text](https://github.com/GDDario/ascender-blog/blob/main/assets/small-logo-white-white-text-and-transparent-bg.png.png)

# Ascender Blog

This is a backend-focused project with the objective of being a study project and maybe a future blogging platform.

Some of the study objectives are:

- Practice programming, in this ai-focused era;
- Deepen my knowledge in PHP;
- Practice some DDD concepts;
- Apply the most used design patterns;
- Use AI to generate a VueJS front-end and understanding it;
- And so on...
